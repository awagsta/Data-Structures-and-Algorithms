...
bool IntSet::isSubsetOf(const IntSet& otherIntSet) const
{
   ...
   for (int i = 0; i < used; ++i)
   {
      isThere = otherIntSet.contains(data[i]);
      if (isThere == false)
         return false;
   }
   return true;
...
{
   IntSet temp = *this;
   int otherSize = otherIntSet.used;
...
{
   bool alreadyThere = contains(anInt);
...
{
   int i = 0; // variable to contain array index
   bool found = false;
   ...
   return false; // if the set does not contain anInt
}
